...
IPV4: 
必须有四个block, 每个block必须用. 隔开, 每个block内数字必须是[0,255]，不满足一条就不符

IPV6:
必须有8个block, 每个block必须用;隔开, 每个block内数字必须是十六进制, 且每个block的size 是[1,4], 没有数字范围限制， 不满足一条就不符
...
public:
    ...
    bool IsValidIPv4(const string & block){
        ...
        for(int i = 0; i<block.size(); i++){
            char c = block[i];
            if(!isdigit(c) || i == 0 && block.size()>1 && block[i] == '0') {
                return false;
            }
            n = n * 10;
            n += block[i] - '0';
    ...
    bool IsValidIPv6(const string & block){
        ...
        for(int i = 0; i<block.size(); i++){
            if(!isxdigit(block[i])) return false;
        }
        return true;
    }
