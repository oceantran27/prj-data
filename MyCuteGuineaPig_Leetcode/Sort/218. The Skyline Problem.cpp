...
Case 1 两个柱子一样高
 ...
 5 _____(a)___
  ...
  1     3      6
   ...
   - 在a点插入第二个 柱子时, 因为第二个柱子没有大于 preh（插入前pq的最大值 == 新插入值）, 所以不会更新res
   - 在pop 时候, 因为Priority_queue 是[[5,6],[5,3]], 所以r=6, 
    3就被pop 出去，而不会被加到res
...
public:
    vector<pair<int, int>> getSkyline(vector<vector<int>>& buildings) {
        ...
        for(auto p:buildings){
          ...
          //can't put (p[0],p[2]) as (p[0],-p[2]), 因为要保证两个interval overlap 时候, 错误的update boundary point
         //比如[[0,2,3],[2,5,3]]， 在2的时候，不能update [2,0] or [2,3], 
