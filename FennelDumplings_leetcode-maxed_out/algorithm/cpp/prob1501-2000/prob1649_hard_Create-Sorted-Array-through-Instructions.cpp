...
#include <algorithm>
// prob1649: Create Sorted Array through Instructions
...
public:
    int createSortedArray(vector<int>& instructions) {
        ...
        sbt.insert(instructions[0]);
        ll ans = 0;
        for(int i = 1; i < n; ++i)
        {
            ll x = sbt.lessthan(instructions[i]);
            ll y = sbt.greaterthan(instructions[i]);
            sbt.insert(instructions[i]);
            ans = (ans + min(x, y)) % MOD;
        }
        return ans;
    }
