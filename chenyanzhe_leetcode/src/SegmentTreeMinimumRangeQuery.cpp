...
void SegmentTreeMinimumRangeQuery::constructMinSegmentTree(
        const vector<int> &input, int low, int high, int pos) {
    if (low == high) {
        segmentTree[pos] = low;
        return;
    }
...
int SegmentTreeMinimumRangeQuery::rangeMinimumQuery(const vector<int> &input,
                                                    int qlow, int qhigh) {
    return rangeMinimumQuery(input, 0, input.size() - 1, qlow, qhigh, 0);
}

int SegmentTreeMinimumRangeQuery::rangeMinimumQuery(const vector<int> &input,
                                                    int low, int high, int qlow, int qhigh, int pos) {
    // total overlap
