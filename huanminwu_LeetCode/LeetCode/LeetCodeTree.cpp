...
int  LeetCodeTree::lastMarkedNodesBFS(queue<int>& queue, vector<vector<int>>& neighbors, vector<int>& dist)
{
    int node = -1;
    int distance = 0;
    while (!queue.empty())
...
/// 5. The input is generated such that edges represents a valid tree.
/// </summary>
vector<int> LeetCodeTree::lastMarkedNodes(vector<vector<int>>& edges)
{
    int n = edges.size() + 1;
    vector<int> result(n);
    vector<vector<int>> neighbors(n);
    ...
    int u = lastMarkedNodesBFS(q, neighbors, dist1);
    dist1 = vector<int>(n, -1);
    q.push(u);
    dist1[u] = 0;
    int v = lastMarkedNodesBFS(q, neighbors, dist1);
    result[0] = u;
    result[u] = v;
    q.push(v);
    dist2[v] = 0;
    lastMarkedNodesBFS(q, neighbors, dist2);
    for (int i = 0; i < n; i++)
    {
        if (dist1[i] > dist2[i])
        {
...
/// </summary>
pair<int, int> LeetCodeTree::kthLargestPerfectSubtree(TreeNode* node, priority_queue<int>& pq, int k)
{
    if (node == nullptr) return { 0, 1 };
    pair<int, int> left = kthLargestPerfectSubtree(node->left, pq, k);
    pair<int, int> right = kthLargestPerfectSubtree(node->right, pq, k);
    pair<int, int> result;
    if (left.first == right.first && left.second == 1 && right.second == 1)
    {
        result.second = 1;
    }
    result.first = 1 + left.first + right.first;
    if (result.second == 1) pq.push(-result.first);
...
/// 3. 1 <= k <= 1024
/// </summary>
int LeetCodeTree::kthLargestPerfectSubtree(TreeNode* root, int k)
{
    priority_queue<int> pq;
    kthLargestPerfectSubtree(root, pq, k);
    if (pq.size() == k)
    {
        return -pq.top();
    }
...
/// </summary>
int LeetCodeTree::findSubtreeSizes(int node, vector<vector<int>>& children, vector<int>& parent, string& s, vector<int>& result)
{
    int origin_parent = parent[s[node] - 'a'];
    parent[s[node] - 'a'] = node;
    for (size_t i = 0; i < children[node].size(); i++)
    {
        int child = children[node][i];
        findSubtreeSizes(child, children, parent, s, result);
        if (parent[s[child] - 'a'] == -1 || parent[s[child] - 'a'] == node)
        {
            result[node] += result[child];
        }
        else
        {
            result[parent[s[child] - 'a']] += result[child];
        }
    }
    parent[s[node] - 'a'] = origin_parent;
    result[node]++;
    return result[node];
}
...
/// </summary>
vector<int> LeetCodeTree::findSubtreeSizes(vector<int>& parent, string s)
{
    int n = parent.size();
    vector<vector<int>> children(n);
    for (size_t i = 1; i < parent.size(); i++)
    ...
    vector<int> result(n), parent_nodes(26, -1);
    findSubtreeSizes(0, children, parent_nodes, s, result);
    return result;
}
...
/// Medium
/// </summary>
int LeetCodeTree::maxTargetNodesI(vector<vector<int>>& neighbors, int parent, int node, int k)
{
    if (k < 0) return 0;
    if (k == 0) return 1;
    int count = 1;
    ...
    {
        if (neighbors[node][i] == parent) continue;
        count += maxTargetNodesI(neighbors, node, neighbors[node][i], k - 1);
    }
    return count;
}
...
/// 10. 0 <= k <= 1000
/// </summary>
vector<int> LeetCodeTree::maxTargetNodesI(vector<vector<int>>& edges1, vector<vector<int>>& edges2, int k)
{
    vector<vector<int>> neighbors1(edges1.size() + 1), neighbors2(edges2.size() + 1);
    for (size_t i = 0; i < edges1.size(); i++)
    {
        neighbors1[edges1[i][0]].push_back(edges1[i][1]);
        neighbors1[edges1[i][1]].push_back(edges1[i][0]);
    }
    for (size_t i = 0; i < edges2.size(); i++)
    {
        neighbors2[edges2[i][0]].push_back(edges2[i][1]);
        neighbors2[edges2[i][1]].push_back(edges2[i][0]);
    }
    int max_count = 0;
    for (size_t i = 0; i < edges2.size() + 1; i++)
    {
        max_count = max(max_count, maxTargetNodesI(neighbors2, -1, i, k - 1));
    }
    vector<int> result(edges1.size() + 1);
    for (size_t i = 0; i < edges1.size() + 1; i++)
    {
        result[i] = maxTargetNodesI(neighbors1, -1, i, k) + max_count;
    }
    return result;
}
...
///                 Trees II
/// </summary>
vector<int> LeetCodeTree::maxTargetNodesII(vector<vector<int>>& neighbors, vector<int>& parity)
{
    queue<int> queue;
    queue.push(0);
    int id = 0;
    ...
    while (!queue.empty())
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            for (size_t j = 0; j < neighbors[node].size(); j++)
            {
                if (parity[neighbors[node][j]] != -1) continue;
                queue.push(neighbors[node][j]);
            }
        }
        id = 1 - id;
    }
...
///    trees.
/// </summary>
vector<int> LeetCodeTree::maxTargetNodesII(vector<vector<int>>& edges1, vector<vector<int>>& edges2)
{
    vector<vector<int>> neighbors1(edges1.size() + 1), neighbors2(edges2.size() + 1);
    vector<int> parity1(edges1.size() + 1, -1), parity2(edges2.size() + 1, -1);
    ...
    count1 = maxTargetNodesII(neighbors1, parity1);
    count2 = maxTargetNodesII(neighbors2, parity2);
    vector<int> result(edges1.size() + 1);
    for (size_t i = 0; i < parity1.size(); i++)
    {
        result[i] = count1[parity1[i]] + max(count2[0], count2[1]);
    }
    return result;
}
...
/// </summary>
pair<long long, long long> LeetCodeTree::maximizeSumOfWeights(vector<vector<pair<int, int>>>& children, int parent, int node, int k)
{
    long long base_sum = 0;
    priority_queue<long long> pq;
    for (size_t i = 0; i < children[node].size(); i++)
    {
        pair<int, int> child = children[node][i];
        if (child.first == parent) continue;
        pair<long long, long long>  child_sum = maximizeSumOfWeights(children, node, child.first, k);
        base_sum += child_sum.second;
        if (child.second + child_sum.first > child_sum.second)
        {
            pq.push(child.second + child_sum.first - child_sum.second);
        }
    }
    long long with_sum = base_sum;
    long long without_sum = base_sum;
    for (int i = 0; i < k; i++)
    {
        if (pq.empty()) break;
        if (i < k - 1)
    ...
    return make_pair(with_sum, without_sum);
}
...
/// </summary>
long long LeetCodeTree::maximizeSumOfWeights(vector<vector<int>>& edges, int k)
{
    vector<vector<pair<int, int>>> children(edges.size() + 1);
    for (size_t i = 0; i < edges.size(); i++)
    {
        children[edges[i][0]].push_back(make_pair(edges[i][1], edges[i][2]));
        children[edges[i][1]].push_back(make_pair(edges[i][0], edges[i][2]));
    }
    pair<long long, long long> result = maximizeSumOfWeights(children, -1, 0, k);
    return max(result.first, result.second);
}
...
/// </summary>
void LeetCodeTree::longestSpecialPath(int parent, int node, 
    vector<int>& nums,
    ...
    vector<int>& result)
{
    for (size_t i = 0; i < neighbors[node].size(); i++)
    {
        ...
        {
            index = max(positions[nums[child.first]].back() + 1, start_index);
        }
        length = prefix_sum.back() - prefix_sum[index];
        count = prefix_sum.size() - index;
        if (length > result[0])
        {
            result[0] = length;
            result[1] = count;
        ...
        longestSpecialPath(node, child.first, nums, neighbors, prefix_sum, index, positions, result);
    }
...
/// </summary>
vector<int> LeetCodeTree::longestSpecialPath(vector<vector<int>>& edges, vector<int>& nums)
{
