...
/// <summary>
/// Leet Code 3369. Design an Array Statistics Tracker 
/// 
/// Hard
///
/// Design a data structure that keeps track of the values in it and 
/// answers some queries regarding their mean, median, and mode.
///
/// Implement the StatisticsTracker class.
///
/// StatisticsTracker(): Initialize the StatisticsTracker object with 
/// an empty array.
...
/// Note:
///
/// The mean of an array is the sum of all the values divided by the 
...
/// // The data structure now contains [4]
/// statisticsTracker.addNumber(4); 
/// // The data structure now contains [4, 4]
/// statisticsTracker.addNumber(2); 
/// // The data structure now contains [4, 4, 2]
/// statisticsTracker.addNumber(3); 
/// // The data structure now contains [4, 4, 2, 3]
...
/// // The data structure now contains [4, 2, 3]
...
/// // The data structure now contains [9, 5]
/// statisticsTracker.getMean(); // return 7
/// statisticsTracker.removeFirstAddedNumber(); 
/// // The data structure now contains [5]
/// statisticsTracker.addNumber(5); 
/// // The data structure now contains [5, 5]
/// statisticsTracker.addNumber(6); 
/// // The data structure now contains [5, 5, 6]
/// statisticsTracker.removeFirstAddedNumber(); 
/// // The data structure now contains [5, 6]
/// statisticsTracker.getMedian(); // return 6
/// statisticsTracker.addNumber(8); 
/// // The data structure now contains [5, 6, 8]
/// statisticsTracker.getMode(); // return 5
/// 
/// Constraints:
/// 1. 1 <= number <= 10^9
/// 2. At most, 10^5 calls will be made to addNumber, 
///    removeFirstAddedNumber, getMean, getMedian, and getMode in total.
/// 3. removeFirstAddedNumber, getMean, getMedian, and getMode will be 
///    called only if there is at least one element in the data structure.
/// </summary>
class StatisticsTracker 
{
private:
    unordered_map<int, int> m_frequency;
    set<pair<int, int>> m_freq_sorted;
    deque<pair<int, int>> m_numbers;
    long long m_sum;
    int m_count;
    int m_id;
    set<pair<int, int>> m_low, m_high;
public:
    StatisticsTracker() 
    {
        m_sum = m_count = m_id = 0;
    }
    ...
    void addNumber(int number) 
    {
        ...
        m_count++;
        m_sum += number;
        pair<int, int> pair = make_pair(number, m_id);
        m_numbers.push_back(pair);
        if (m_frequency.count(number) > 0)
        {
            m_freq_sorted.erase(make_pair(0 - m_frequency[number], number));
        }
        m_frequency[number]++;
        m_freq_sorted.insert(make_pair(0 - m_frequency[number], number));
        if (!m_low.empty() && m_low.rbegin()->first > pair.first)
        {
            m_low.insert(pair);
            pair = *m_low.rbegin();
            m_low.erase(pair);
        ...
        m_high.insert(pair);
        if (m_high.size() > m_low.size() + 1)
        {
            m_low.insert(*m_high.begin());
    ...
    {
        pair<int, int> pair = m_numbers.front();
        m_numbers.pop_front();
        m_count--;
        ...
        m_freq_sorted.erase(make_pair(0 - m_frequency[number], number));
        m_frequency[number]--;
        if (m_frequency[number] > 0)
        {
            m_freq_sorted.insert(make_pair(0 - m_frequency[number], number));
        }
        else
        {
            m_frequency.erase(number);
        }
        m_high.erase(pair);
        m_low.erase(pair);
        if (m_high.size() < m_low.size())
        {
            m_high.insert(*m_low.rbegin());
            m_low.erase(*m_low.rbegin());
        }
        else if (m_high.size() > m_low.size() + 1)
        {
            m_low.insert(*m_high.begin());
            m_high.erase(m_high.begin());
        }
    ...
    int getMean() 
    {
        return (int)(m_sum / m_count);
    }

    int getMedian() 
    {
        return m_high.begin()->first;
    }

    int getMode() 
    {
        return m_freq_sorted.begin()->second;
    }
};


/// <summary>
/// Leet Code 3391. Design a 3D Binary Matrix with Efficient Layer Tracking
/// 
/// 
/// Medium
///
...
///
/// Explanation
/// matrix3D matrix3D = new matrix3D(3); // Initializes a 3 x 3 x 3 3D 
/// array matrix, filled with all 0's.
/// matrix3D.setCell(0, 0, 0); // Sets matrix[0][0][0] to 1.
/// matrix3D.largestMatrix(); 
/// // Returns 0. matrix[0] has the most number of 1's.
/// matrix3D.setCell(1, 1, 2); // Sets matrix[1][1][2] to 1.
/// matrix3D.largestMatrix(); // Returns 1. matrix[0] and matrix[1] 
...
/// [null, null, 2, null, 3]
/// Explanation
/// matrix3D matrix3D = new matrix3D(4); // Initializes a 4 x 4 x 4 3D 
/// // array matrix, filled with all 0's.
/// matrix3D.setCell(2, 1, 1); // Sets matrix[2][1][1] to 1.
/// matrix3D.largestMatrix(); 
/// // Returns 2. matrix[2] has the most number of 1's.
/// matrix3D.unsetCell(2, 1, 1); // Sets matrix[2][1][1] to 0.
/// matrix3D.largestMatrix(); // Returns 3. All indices from 0 to 3 tie 
/// with the same number of 1's, but index 3 is the biggest.
/// 
/// Constraints:
/// 1. 1 <= n <= 100
/// 2. 0 <= x, y, z < n
/// 3. At most 10^5 calls are made in total to setCell and unsetCell.
/// 4. At most 10^4 calls are made to largestMatrix.
/// </summary>
class Matrix3D
{
private:
    vector<vector<vector<int>>> m_matrix;
    vector<int> m_page;
    set<pair<int, int>> pq;
  public:
    Matrix3D(int n)
    {
        m_matrix = vector<vector<vector<int>>>(n, vector<vector<int>>(n, vector<int>(n)));
        m_page = vector<int>(n);
    }
    ...
    void setCell(int x, int y, int z)
    {
        if (m_matrix[x][y][z] == 0)
        {
            if (m_page[x] > 0)
            {
                pq.erase(make_pair(m_page[x], x));
            }
            m_page[x]++;
            pq.insert(make_pair(m_page[x], x));
        }
        m_matrix[x][y][z] = 1;
    }
