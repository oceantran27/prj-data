...
/// </summary>
int LeetCodeHashtable::distributeCandies(vector<int>& candies)
{
...
/// 2. 1 <= nums[i] <= 100
/// </summary>
int LeetCodeHashtable::maxFrequencyElements(vector<int>& nums)
{
    unordered_map<int, int> num_count;
    for (size_t i = 0; i < nums.size(); i++)
    {
...
/// <summary>
/// Leet Code 3016. Minimum Number of Pushes to Type Word II
///                 
/// Medium
///
...
/// "i" -> one push on key 9
/// Total cost is 1 * 2 + 1 * 2 + 1 * 2 + 1 * 2 + 1 * 2 + 1 * 2 + 1 * 2 
/// + 2 * 2 + 6 * 1 = 24.
/// It can be shown that no other mapping can provide a lower cost.
/// 
/// Constraints:
...
/// </summary>
int LeetCodeHashtable::minimumPushesII(string word)
{
    vector<int> dp(26);
    for (size_t i = 0; i < word.size(); i++)
    {
...
/// <summary>
/// Leet Code 3020. Find the Maximum Number of Elements in Subset
///
/// Medium
///
...
/// (Note that k can be be any non-negative power of 2). For example, [2, 4, 
/// 16, 4, 2] and [3, 9, 3] follow the pattern while [2, 4, 8, 4, 2] does not.
...
/// the array as [2,4,2] which follows the pattern and 22 == 4. Hence the 
/// answer is 3.
///
/// Example 2:
/// Input: nums = [1,3,2,4]
...
/// array as [1] which follows the pattern. Hence the answer is 1. Note 
/// that we could have also selected the subsets {2}, {4}, or {3}, there 
/// may be multiple subsets which provide the same answer. 
/// 
/// Constraints:
/// 1. 2 <= nums.length <= 10^5
/// 2. 1 <= nums[i] <= 10^9
/// </summary>
int LeetCodeHashtable::maximumLength(vector<int>& nums)
{
    map<int, int> num_map;
    ...
    while (!num_map.empty())
    {
        int num = num_map.begin()->first;
        int count = min(2, num_map.begin()->second);
        ...
        while (count > 0)
        {
            sum += count;
            num_map[num] -= count;
            if (num_map[num] == 0) num_map.erase(num);
            visited.insert(num);
            if (count == 2)
            {
                count = 0;
                num = (int)((long long)num * (long long)num);
                if (num_map.count(num) > 0)
                {
                    count = min(2, num_map[num]);
                }
                else
                {
                    sum--;
...
/// <summary>
/// Leet Code 3046. Split the Array
///
/// Easy
///
...
/// 3. 1 <= nums[i] <= 100
/// </summary>
bool LeetCodeHashtable::isPossibleToSplit(vector<int>& nums)
{
    unordered_map<int, int> hash_table;
    for (int num : nums) hash_table[num]++;
...
/// <summary>
/// Leet 3092. Most Frequent IDs
///
/// Medium
///
/// The problem involves tracking the frequency of IDs in a collection 
/// that changes over time. You have two integer arrays, nums and freq, 
/// of equal length n. Each element in nums represents an ID, and the 
/// corresponding element in freq indicates how many times that ID should 
/// be added to or removed from the collection at each step.
///
...
/// the most frequent ID in the collection after the ith step. If the 
...
/// After step 1, we have 3 IDs with the value of 2 and 2 IDs with the 
/// value of 3. So ans[1] = 3.
/// After step 2, we have 2 IDs with the value of 3. So ans[2] = 2.
/// After step 3, we have 2 IDs with the value of 3 and 1 ID with the 
/// value of 1. So ans[3] = 2.
///
/// Example 2:
...
/// 3. -10^5 <= freq[i] <= 10^5
/// 4. freq[i] != 0
/// 5. The input is generated such that the occurrences of an ID will not 
///    be negative in any step.
/// </summary>
vector<long long> LeetCodeHashtable::mostFrequentIDs(vector<int>& nums, vector<int>& freq)
{
    ...
    for (size_t i = 0; i < nums.size(); i++)
    {
        if (freq_map.count(nums[i]) > 0)
        {
            heap.erase(make_pair(freq_map[nums[i]], nums[i]));
        }
        freq_map[nums[i]] += freq[i];
        if (freq_map[nums[i]] > 0)
        {
            heap.insert(make_pair(freq_map[nums[i]], nums[i]));
        }
        if (heap.empty()) result.push_back(0);
        else result.push_back(heap.rbegin()->first);
    }
    return result;
}
...
/// </summary>
int LeetCodeHashtable::duplicateNumbersXOR(vector<int>& nums)
{
    unordered_map<int, int> num_count;
    for (size_t i = 0; i < nums.size(); i++)
    {
...
/// integer denoting the number of pairs i, j where i < j and 
/// hours[i] + hours[j] forms a complete day.
///
/// A complete day is defined as a time duration that is an exact multiple 
/// of 24 hours.
///
/// For example, 1 day is 24 hours, 2 days is 48 hours, 3 days is 72 
/// hours, and so on.
///
/// Example 1:
...
/// Output: 3
/// Explanation:
/// The pairs of indices that form a complete day are (0, 1), (0, 2), 
/// and (1, 2).
///
/// Constraints:
...
/// 2. 1 <= hours[i] <= 10^9
/// </summary>
int LeetCodeHashtable::countCompleteDayPairsI(vector<int>& hours)
{
    vector<int> dp(24);
    int result = 0;
...
/// 2. 1 <= hours[i] <= 10^9
/// </summary>
long long LeetCodeHashtable::countCompleteDayPairsII(vector<int>& hours)
{
    vector<int> dp(24);
    long long result = 0;
    for (size_t i = 0; i < hours.size(); i++)
...
/// <summary>
/// Leet Code 3289. The Two Sneaky Numbers of Digitville
/// 
/// Easy
///
...
/// 2. nums.length == n + 2
/// 3. 0 <= nums[i] < n
/// 4. The input is generated such that nums contains exactly two repeated 
///    elements.
/// </summary>
vector<int> LeetCodeHashtable::getSneakyNumbers(vector<int>& nums)
{
    ...
    {
        dp[nums[i]]++;
        if (dp[nums[i]] == 2) result.push_back(nums[i]);
    }
    return result;
}
#pragma endregion
