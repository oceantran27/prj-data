...
/// 2. 1 <= Node.val <= 10^5
/// </summary>
vector<int> LeetCodeLinkedList::nodesBetweenCriticalPoints(ListNode* head)
{
    ...
    while (node != nullptr)
    {
        ...
        {
            if ((prev_node->val < node->val && next_node->val < node->val) ||
                (prev_node->val > node->val && next_node->val > node->val))
            {
                if (first_id != -1)
                {
                    result[1] = max(result[1], id - first_id);
                }
                else
                {
                    first_id = id;
                ...
                if (prev_id != -1)
                {
                    result[0] = min(result[0], id - prev_id);
                }
                prev_id = id;
            }
        }
    ...
    if (result[1] == INT_MIN) result[1] = -1;
    return result;
}
...
/// 2. 1 <= Node.val <= 10^5
/// </summary>
int LeetCodeLinkedList::pairSum(ListNode* head)
{
    ListNode* slow = head;
    ListNode* fast = head;
    int count = 0;
    ...
    ListNode* start = ((slow == nullptr) ? slow : slow->next);
    ...
    start = head;
    slow = ((slow == nullptr) ? slow : slow->next);
    int result = 0;
    while (slow != nullptr)
    {
        result = max(result, (start == nullptr ? 0 : start->val) + (slow == nullptr ? 0 : slow->val));
        start = start->next;
        slow = slow->next;
    }
    return result;
...
/// </summary>
ListNode* LeetCodeLinkedList::mergeNodes(ListNode* head)
{
    ListNode* node = head;
...
/// 2. 1 <= Node.val <= 10^5
/// </summary>
ListNode* LeetCodeLinkedList::removeNodes(ListNode* head)
{
    ListNode* prev = nullptr;
    while (head != nullptr)
    {
        while (prev != nullptr && head->val > prev->val)
        {
            prev = prev->next;
        }
        ListNode* next = head->next;
...
/// </summary>
vector<ListNode*> LeetCodeLinkedList::splitCircularLinkedList(ListNode* list)
{
    ...
    slow->next = result[0];
    if (fast->next == list) fast->next = result[1];
    else
    {
        if (fast->next->next == list) fast->next->next = result[1];
    }
    return result;
}
...
/// 2. 1 <= Node.val <= 1000
/// </summary>
ListNode* LeetCodeLinkedList::insertGreatestCommonDivisors(ListNode* head)
{
    ListNode* curr = head;
...
/// </summary>
ListNode* LeetCodeLinkedList::doubleIt(ListNode* head)
{
    head = reverseList(head);
    int carry = 0;
    ListNode* prev = head;
...
/// </summary>
string LeetCodeLinkedList::gameResult(ListNode* head)
{
    ListNode* curr = head;
    int odd = 0, even = 0;
    while (curr != nullptr)
...
/// 2. 1 <= Node.val <= 105
/// </summary>
ListNode* LeetCodeLinkedList::frequenciesOfElements(ListNode* head)
{
    ...
    while (curr != nullptr)
    {
        if (frequency.count(curr->val) != 0)
        {
            target = new ListNode(frequency[curr->val]);
            if (prev != nullptr) prev->next = target;
            prev = target;
            if (result == nullptr) result = target;
            frequency.erase(curr->val);
        }
        curr = curr->next;
    }
    return result;
