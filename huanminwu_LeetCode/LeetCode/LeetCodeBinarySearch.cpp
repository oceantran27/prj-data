...
/// 2. 1 <= x <= 8
/// </summary>
long long LeetCodeBinarySearch::findMaximumNumber(long long k, int x)
{
    long long first = 0;
    ...
    while (first <= last)
    {
        ...
        {
            if ((i + 1) % x != 0) continue;
            if (((long long)1 << i) > middle) break;
            count += middle / ((long long)1 << (i + 1)) * ((long long)1 << i);
            count += max((long long)0, middle % ((long long)1 << (i + 1)) - ((long long)1 << i));
            if (count > k) break;
        }
        if (count > k)
        {
...
/// </summary>
int LeetCodeBinarySearch::earliestSecondToMarkIndicesI(vector<int>& nums, vector<int>& changeIndices)
{
    int first = 1;
    int last = changeIndices.size();
    int result = -1;
    while (first <= last)
    {
        int middle = (first + last) / 2;
        vector<int> time = nums;
        vector<int> changes(middle);
        int count = 0;
        int busy_slot = 0;
        for (int i = middle-1; i >= 0; i--)
        {
            if (time[changeIndices[i] - 1] != -1)
            {
                busy_slot += time[changeIndices[i] - 1];
                time[changeIndices[i] - 1] = -1;
                changes[i] = changeIndices[i] - 1;
                count++;
            }
            else
            {
                if (busy_slot > 0) busy_slot--;
                changes[i] = -1;
            }
        }
        if (count == nums.size() && busy_slot == 0)
        {
            result = middle;
            last = middle - 1;
        }
...
/// 4. 1 <= changeIndices[i] <= n
/// </summary>
int LeetCodeBinarySearch::earliestSecondToMarkIndicesII(vector<int>& nums, vector<int>& changeIndices)
{
    int first = 1;
    int last = changeIndices.size();
    int result = -1;
    while (first <= last)
    {
        ...
        {
            if (time[changeIndices[i] - 1] != 0)
            {
                time[changeIndices[i] - 1] = 0;
                changes.push_back(changeIndices[i] - 1);
            }
            else
            {
                changes.push_back(-1);
        ...
        for (int i = middle - 1; i >= 0; i--)
        {
            if (changes[i] == -1) free_slot++;
            else
            {
                pq.insert(make_pair(nums[changes[i]], i));
                // if we have free_slot we can make this element zero, 
                // otherwise we should take out the cheapest element.
        ...
        while (!pq.empty())
        {
            process_time -= pq.begin()->first - 1;
            pq.erase(pq.begin());
        }
        if (process_time <= (long long)middle)
        {
            result = middle;
            last = middle - 1;
        }
...
int LeetCodeBinarySearch::medianOfUniquenessArray(vector<int>& nums)
{
    ...
    while (first <= last)
    {
        ...
        {
            num_count[nums[j]]++;
            while ((int)num_count.size() > middle)
            {
                num_count[nums[i]]--;
