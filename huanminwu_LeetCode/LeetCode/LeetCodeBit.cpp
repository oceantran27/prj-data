...
/// </summary>
string LeetCodeBit::maximumOddBinaryNumber(string s)
{
    int one = 0;
    for (size_t i = 0; i < s.size(); i++)
    {
    ...
    {
        if (i < one - 1) result.push_back('1');
        else if (i == s.size() - 1) result.push_back('1');
...
/// 2. 1 <= nums[i] <= 10^9
/// </summary>
int LeetCodeBit::maxSum(vector<int>& nums, int k)
{
    long long M = 1000000007;
    vector<int> num_bits(32);
    for (size_t i = 0; i < nums.size(); i++)
    {
        for (int j = 0; j < 32; j++)
        {
    ...
    for (int i = 0; i < k; i++)
    {
        ...
        for (int j = 0; j < 32; j++)
        {
            if (num_bits[j] > i) num |= (1 << j);
        }
        result = (result + (long long)num * (long long)num) % M;
...
/// 3. 1 <= k <= nums.length
/// </summary>
int LeetCodeBit::findKOr(vector<int>& nums, int k)
{
    vector<int> dp(32);
    int result = 0;
    for (size_t i = 0; i < nums.size(); i++)
...
/// 2. 1 <= nums[i] <= 100
/// </summary>
int LeetCodeBit::maximumStrongPairXorI(vector<int>& nums)
{
    sort(nums.begin(), nums.end());
    int result = 0;
    int prev = 0;
...
/// 2. 1 <= nums[i] <= 2^20 - 1
/// </summary>
int LeetCodeBit::maximumStrongPairXorII(vector<int>& nums)
{
    sort(nums.begin(), nums.end());
    TrieBitNode* trieBitNode = new TrieBitNode(20);
    int result = 0;
    int prev = 0;
    int n = nums.size();
    for (int i = 0; i < n; i++)
    {
        while (nums[prev] * 2 < nums[i])
        {
...
/// 2. 0 <= n <= 50
/// </summary>
int LeetCodeBit::maximumXorProduct(long long a, long long b, int n)
{
    long long M = 1000000007;
    for (int i = n - 1; i >= 0; i--)
    {
        long long bit = ((long long)1 << (long long)i);
        if (min(a ^ bit, b ^ bit) > min(a, b))
        {
            a = a ^ bit;
            b = b ^ bit;
        }
...
/// 3. 0 <= k < nums.length
/// </summary>
int LeetCodeBit::minOrAfterOperations(vector<int>& nums, int k)
{
    int result = 0;
    int bit_mask = 0;
    for (int i = 30; i >= 0; i--) 
    {
        bit_mask = result | (1 << i);
        int cosecutiveAnd = bit_mask;
        int count = 0;
        for (int num : nums) 
        {
            cosecutiveAnd = cosecutiveAnd & num;
            if (cosecutiveAnd  != 0)
            {
                count++;
            }
...
/// 3. 0 <= nums[i] < 2^m
/// </summary>
vector<int> LeetCodeBit::maxHammingDistances(vector<int>& nums, int m)
{
    ...
    for (size_t i = 0; i < nums.size(); i++)
    {
        distances[nums[i] ^ bit_max] = 0;
        queue.push(nums[i] ^ bit_max);
    }
    while (!queue.empty())
    {
        int n = queue.front();
...
/// 2. 0 <= a[i], b[i], c[i] <= 100
/// </summary>
int LeetCodeBit::tripletCountI(vector<int>& a, vector<int>& b, vector<int>& c)
{
    int result = 0;
    for (size_t i = 0; i < a.size(); i++)
    {
...
/// 2. 0 <= nums[i], k <= 10^9
/// </summary>
long long LeetCodeBit::countSubarrays(vector<int>& nums, int k)
{
    int prev = -1;
    int n = nums.size();
    vector<int> dp(31, n);
    int bit_mask = INT_MAX;
    ...
    for (size_t i = 0; i < nums.size(); i++)
    {
        ...
        if (bit_mask != k) continue;
        int start = n;
        for (int j = 0; j < 31; j++)
        {
            if (dp[j] < n) start = min(start, dp[j]);
        }
...
/// </summary>
long long LeetCodeBit::tripletCountII(vector<int>& a, vector<int>& b, vector<int>& c)
{
    ...
    long long result = 0;
    result += (long long)dp[0][0] * (long long)dp[1][0] * (long long)dp[2][0];
    result += (long long)dp[0][0] * (long long)dp[1][1] * (long long)dp[2][1];
    result += (long long)dp[0][1] * (long long)dp[1][0] * (long long)dp[2][1];
    result += (long long)dp[0][1] * (long long)dp[1][1] * (long long)dp[2][0];
    return result;
}
...
/// 1. 1 <= n, k <= 10^6
/// </summary>
int LeetCodeBit::minChanges(int n, int k)
{
    int result = 0;
    for (int i = 0; i < 31; i++)
    {
...
/// 1. 1 <= n <= 1000
/// </summary>
int LeetCodeBit::smallestNumber(int n)
{
    int result = 1;
    while (result <= n)
    {
