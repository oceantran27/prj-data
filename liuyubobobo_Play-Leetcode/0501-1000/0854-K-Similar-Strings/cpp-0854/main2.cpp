...
/// In this code, I optimized the contains_circle function and get the circle number as returns:)
///
...
private:
    ...
    int max_circle_num(vector<int>& g, const vector<string>& circles){
        ...
        for(const string& circle: circles)
            if(int k = contains_circle(g, circle)){
                ...
                res = max(res, k + max_circle_num(g, circles));
    ...
    int contains_circle(const vector<int> g, const string& circle){
        int res = INT_MAX;
        for(int i = 0 ; i < circle.size() ; i ++){
            int u = circle[i] - 'a';
            int v = circle[(i + 1) % circle.size()] - 'a';
