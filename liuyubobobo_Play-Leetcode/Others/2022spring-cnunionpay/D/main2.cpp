...
private:
    ...
    vector<long long> h1, h4;
    vector<vector<long long>> h2;

public:
    Solution(): h1(1001), h2(1001, vector<long long>(1001)){

        for(int i = 0; i <= 1000; i ++) h1[i] = i;
        for(int i = 0; i <= 1000; i ++)
            for(int j = 0; j <= 1000; j ++) h2[i][j] = i * 1001 + j;
    }
    ...
    long long contains(const unordered_map<long long, int> & f,
        ...
        for(int i = 0; i < skills.size(); i ++)
            for(int j = i + 1; j < skills.size(); j ++){
                auto iter = f.find(h2[skills[i]][skills[j]]);
                res += iter == f.end() ? 0 : iter->second;
            }
