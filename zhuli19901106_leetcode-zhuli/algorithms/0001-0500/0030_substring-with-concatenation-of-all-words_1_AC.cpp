...
public:
    ...
    vector<int> findSubstring(string s, vector<string>& words) {
        ...
        unordered_map<uint64_t, int> cnt1;
        int lw = w[0].size();
        uint64_t hw = 0;
        int i, j;
        for (i = 0; i < nw; ++i) {
        ...
        vector<uint64_t> hs(ls + 2, 0);
        for (i = 1; i <= ls; ++i) {
            hs[i] = hs[i - 1] * P + (s[i - 1] - 'a' + 1);
        }
        
        unordered_map<uint64_t, int> cnt2;
        int k;
        uint64_t old_hw;
        for (k = 0; k < lw; ++k) {
            i = k;
...
private:
    static const uint64_t P = 31;
    vector<uint64_t> pw;
    
    uint64_t getPow(int i) {
        while (pw.size() <= i) {
    ...
    uint64_t hash(vector<uint64_t> &h, int left, int right) {
